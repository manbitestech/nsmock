...
describe('N/http module', () => {
    ...
    describe('_setResponse / _getResponse', () => {
        test('should store and retrieve a response keyed by method and url', () => {
            http._setResponse('GET', 'https://api.example.com/orders', { code: 200, body: 'ok', headers: { 'content-type': 'application/json' } });
            expect(http._getResponse('GET', 'https://api.example.com/orders')).toEqual({ code: 200, body: 'ok', headers: { 'content-type': 'application/json' } });
        });

        test('should distinguish responses by method for the same url', () => {
            http._setResponse('GET', 'https://api.example.com/orders', { code: 200, body: 'get' });
            http._setResponse('POST', 'https://api.example.com/orders', { code: 201, body: 'post' });
            expect(http._getResponse('GET', 'https://api.example.com/orders').body).toBe('get');
            expect(http._getResponse('POST', 'https://api.example.com/orders').body).toBe('post');
        });

        test('_clearResponses should empty the store', () => {
            http._setResponse('GET', 'https://api.example.com/orders', { code: 200, body: 'ok' });
            http._clearResponses();
            expect(http._getResponse('GET', 'https://api.example.com/orders')).toBeUndefined();
        });
    });

    describe('get / post / put / delete', () => {
        test('get should return the configured response', () => {
            http._setResponse('GET', 'https://api.example.com/orders', { code: 200, body: '{"id":1}', headers: { 'content-type': 'application/json' } });
        ...
        test('post should return the configured response', () => {
            http._setResponse('POST', 'https://api.example.com/orders', { code: 201, body: 'created' });
        ...
        test('put should return the configured response', () => {
            http._setResponse('PUT', 'https://api.example.com/orders/1', { code: 200, body: 'updated' });
        ...
        test('delete should return the configured response', () => {
            http._setResponse('DELETE', 'https://api.example.com/orders/1', { code: 204, body: '' });
